...
namespace JuniperMigration
{
    ...
    public class JuniperConverter : VendorConverter
    {
        ...
        private bool IsNetworkObjectReachableFromInterface(CheckPointObject cpObject, Juniper_Interface juniperInterface)
        {
            ...
            var interfaceRoutes = GetJuniperInterfaceRoutes(juniperInterface);
            if (interfaceRoutes != null && interfaceRoutes.Any() && IsNetworkObjectContainedInInterfaceRoutes(cpObject, interfaceRoutes))
            {
                return true;
            }
        ...
        private bool IsNetworkObjectContainedInInterfaceRoutes(CheckPointObject cpObject, IEnumerable<IPNetwork> interfaceRoutes)
        {
            ...
            if (cpNetworkGroup != null)
            {
                foreach (var memberName in cpNetworkGroup.Members)
                {
                    ...
                    if (IsNetworkObjectContainedInInterfaceRoutes(member, interfaceRoutes))
                    {
                        return true;
                    }
                }
        ...
        private List<IPNetwork> GetJuniperInterfaceRoutes(Juniper_Interface ifc)
        {
            ...
            if (ifc.Topology.Count > 0 && !_juniperInterfacesRoutes.TryGetValue(ifc.Name, out interfaceRoutes))
            {
                ...
                {
                    IPNetwork ipNetwork;
                    if (IPNetwork.TryParse(route.IpAddress, route.Netmask, out ipNetwork))
                    {
