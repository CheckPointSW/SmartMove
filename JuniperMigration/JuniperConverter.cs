...
namespace JuniperMigration
{
    ...
    public class JuniperConverter : VendorConverter
    {
        ...
        private Dictionary<string, List<IPNetwork>> _juniperInterfacesRoutes = new Dictionary<string, List<IPNetwork>>();
        ...
        private void Add_Zones()
        {
            foreach (Juniper_Zone zone in JuniperZones)
            {
                ...
                foreach (var ifc in zone.Interfaces)
                {
                    var cpObject = _cpObjects.GetObject(ifc + "_subnets");
                    if (cpObject != null)
                    {
                ...
                foreach (var natRule in natPolicy.Rules)
                {
                    ...
                    switch (natRule.TranslationMode)
                    {
                        case Juniper_SourceNatRule.SourceTranslationMode.Pool:
                            cpNatRule.TranslatedSource = GetNatTranslatedSourceFromJuniperSourceNatRule(natRule, natPool, cpNatRule);
                            break;

                        case Juniper_SourceNatRule.SourceTranslationMode.Interface:
                            cpNatRule.TranslatedDestination = GetNatTranslatedDestinationFromJuniperSourceNatRule(natPolicy, natRule, cpNatRule);
                            break;
                    }
        ...
        private CheckPointObject GetNatSourceFromJuniperNatRule(Juniper_NatPolicy natPolicy, Juniper_NatRule natRule)
        {
            ...
            else   // no source addresses...
            {
                ...
                {
                    if (natPolicy.SourceZones.Count > 1)
                    {
                        // Create a network group object to hold the network groups associated with a source zone.
                        var cpNetworkGroup = new CheckPoint_NetworkGroup();
                        cpNetworkGroup.Name = NetworkObjectNameGenerator.AutoGeneratedNetworkGroupName();

                        foreach (var sourceZone in natPolicy.SourceZones)
                        {
                            string zoneGroupName = "zone_" + sourceZone + "_subnets";
                            var cpObject = GetCheckPointObjectOrCreateDummy(zoneGroupName,
                                                                            "NetworkGroup",
                                                                            natPolicy,
                                                                            "Error creating a NAT rule, missing member of network group object for NAT rule according to Juniper source zone's interfaces information",
                                                                            "Source zone details: " + sourceZone + ".");
                            cpNetworkGroup.Members.Add(cpObject.Name);

                            if (cpObject.Name.Contains(AutoGeneratedNameWithError))
                            {
                                cpNetworkGroup.ConvertedCommandId = natPolicy.LineNumber;
                                cpNetworkGroup.ConversionIncidentType = cpObject.ConversionIncidentType;
                            }
                        }
                ...
                {
                    if (natPolicy.SourceInterfaces.Count > 1)
                    {
                        // Create a network group object to hold the network groups associated with a source interface.
                        var cpNetworkGroup = new CheckPoint_NetworkGroup();
                        cpNetworkGroup.Name = NetworkObjectNameGenerator.AutoGeneratedNetworkGroupName();

                        foreach (var sourceInterface in natPolicy.SourceInterfaces)
                        {
                            string interfaceGroupName = sourceInterface + "_subnets";
                            var cpObject = GetCheckPointObjectOrCreateDummy(interfaceGroupName,
                                                                            "NetworkGroup",
                                                                            natPolicy,
                                                                            "Error creating a NAT rule, missing member of network group object for NAT rule according to Juniper source interface's topology information",
                                                                            "Source interface details: " + sourceInterface + ".");
                            cpNetworkGroup.Members.Add(cpObject.Name);
        ...
        private CheckPointObject GetNatDestinationFromJuniperNatRule(Juniper_NatPolicy natPolicy, Juniper_NatRule natRule)
        {
            ...
            else   // no destination addresses...
            {
                if (natPolicy is Juniper_SourceNatPolicy)
                {
                    ...
                    {
                        if (sourceNatPolicy.DestinationZones.Count > 1)
                        {
                            // Create a network group object to hold the network groups associated with a destination zone.
                            var cpNetworkGroup = new CheckPoint_NetworkGroup();
                            cpNetworkGroup.Name = NetworkObjectNameGenerator.AutoGeneratedNetworkGroupName();

                            foreach (var destinationZone in sourceNatPolicy.DestinationZones)
                            {
                                string zoneGroupName = "zone_" + destinationZone + "_subnets";
                                var cpObject = GetCheckPointObjectOrCreateDummy(zoneGroupName,
                                                                                "NetworkGroup",
                                                                                natPolicy,
                                                                                "Error creating a NAT rule, missing member of network group object for NAT rule according to Juniper destination zone's interfaces information",
                                                                                "Destination zone details: " + destinationZone + ".");
                                cpNetworkGroup.Members.Add(cpObject.Name);
                    ...
                    {
                        if (sourceNatPolicy.DestinationInterfaces.Count > 1)
                        {
                            // Create a network group object to hold the network groups associated with a destination interface.
                            var cpNetworkGroup = new CheckPoint_NetworkGroup();
                            cpNetworkGroup.Name = NetworkObjectNameGenerator.AutoGeneratedNetworkGroupName();

                            foreach (var destinationInterface in sourceNatPolicy.DestinationInterfaces)
                            {
                                string interfaceGroupName = destinationInterface + "_subnets";
                                var cpObject = GetCheckPointObjectOrCreateDummy(interfaceGroupName,
                                                                                "NetworkGroup",
                                                                                natPolicy,
                                                                                "Error creating a NAT rule, missing member of network group object for NAT rule according to Juniper destination interface's topology information",
                                                                                "Destination interface details: " + destinationInterface + ".");
                                cpNetworkGroup.Members.Add(cpObject.Name);
        ...
        private CheckPointObject GetNatTranslatedSourceFromJuniperSourceNatRule(Juniper_SourceNatRule natRule, IEnumerable<JuniperObject> natPool, CheckPoint_NAT_Rule cpNatRule)
        {
        ...
        private CheckPointObject GetNatTranslatedDestinationFromJuniperSourceNatRule(Juniper_SourceNatPolicy natPolicy, Juniper_SourceNatRule natRule, CheckPoint_NAT_Rule cpNatRule)
        {
            CheckPointObject translatedDestination = null;
            ...
            if (natPolicy.DestinationInterfaces.Count != 0)
            {
                ...
                foreach (var destinationInterface in natPolicy.DestinationInterfaces)
                {
                    foreach (Juniper_Interface ifc in JuniperInterfaces)
                    {
                        if (destinationInterface == ifc.Name)
                        {
                            var interfaceRoutes = GetJuniperInterfaceRoutes(ifc);
                            if (interfaceRoutes != null && interfaceRoutes.Any() && IsNetworkObjectContainedInInterfaceRoutes(cpNatRule.Destination, interfaceRoutes))
                            {
                                translatedDestination = GetCheckPointObjectOrCreateDummy(destinationInterface + "_address",
                                                                                         "Host",
                                                                                         natRule,
                                                                                         "Error creating a NAT rule, missing topology information for Juniper destination interface",
                                                                                         "Destination interface '" + destinationInterface + "' doesn't contain appropriate static routes.");
                            }
                            ...
                            foreach (var ifcName in zone.Interfaces)
                            {
                                foreach (Juniper_Interface ifc in JuniperInterfaces)
                                {
                                    if (ifcName == ifc.Name)
                                    {
                                        ...
                                        if (interfaceRoutes != null && interfaceRoutes.Any() && IsNetworkObjectContainedInInterfaceRoutes(cpNatRule.Destination, interfaceRoutes))
                                        {
                                            translatedDestination = GetCheckPointObjectOrCreateDummy(ifcName + "_address",
                                                                                                     "Host",
                                                                                                     natRule,
                                                                                                     "Error creating a NAT rule, missing topology information for Juniper destination interface",
                                                                                                     "Destination interface '" + ifcName + "' doesn't contain appropriate static routes.");
                                        }
            ...
            if (translatedDestination == null)
            {
                translatedDestination = GetCheckPointObjectOrCreateDummy("not_existing_object",
                                                                         ...
                                                                         natRule,
                                                                         "Error creating a NAT rule, missing topology information for Juniper destination interface",
                                                                         "Cannot find an interface with appropriate static routes.");
            }
                ...
                foreach (CheckPoint_Rule cpParentRule in cpPackage.ParentLayer.Rules)
                {
                    ...
                    foreach (CheckPoint_Layer subPolicy in cpPackage.SubPolicies)
                    {
                        ...
                        for (int ruleNumber = 0; ruleNumber < subPolicy.Rules.Count; ruleNumber++)
                        {
                            ...
                            {
                                string natTranslatedServiceName = (cpNatRule.TranslatedService != null)
                                    ? cpNatRule.TranslatedService.Name
                                    : (cpNatRule.Service != null ? cpNatRule.Service.Name : "");

                                var newRule = new CheckPoint_Rule();

                                newRule.Enabled = cpRule.Enabled;
                                newRule.Source.AddRange(cpRule.Source);
                                newRule.Destination.Add(cpNatRule.TranslatedDestination);
                                if (serviceMatchedToo)
                                {
                                    newRule.Service.Add(_cpObjects.GetObject(natTranslatedServiceName));
                                }
                                else
                                {
        ...
        private bool IsFirewallRuleMatchedByNATRule(CheckPoint_NAT_Rule natRule, CheckPoint_Rule fwRule, out bool serviceMatchedToo)
        {
            ...
            if (fwRuleSourceRanges.Overlaps(natSourceRanges) && fwRuleDestRanges.Overlaps(natDestRanges))
            {
        ...
        private bool IsNetworkObjectContainedInInterfaceRoutes(CheckPointObject natDestination, IEnumerable<IPNetwork> interfaceRoutes)
        {
            if (natDestination == null || natDestination.Name.Contains(AutoGeneratedNameWithError))
            {
                return false;
            }

            var cpNetworkGroup = natDestination as CheckPoint_NetworkGroup;
            if (cpNetworkGroup != null)
            {
                foreach (var member in cpNetworkGroup.Members)
                {
                    var cpObject = _cpObjects.GetObject(member);
                    if (IsNetworkObjectContainedInInterfaceRoutes(cpObject, interfaceRoutes))
                    {
                        return true;
                    }
                }
            ...
            var cpNetwork = natDestination as CheckPoint_Network;
            if (cpNetwork != null)
            {
                IPNetwork network;
                if (IPNetwork.TryParse(cpNetwork.Subnet, cpNetwork.Netmask, out network))
                {
                    return interfaceRoutes.Any(route => IPNetwork.Contains(route, network));
                }

                return false;
            }

            var cpRange = natDestination as CheckPoint_Range;
            if (cpRange != null)
            {
            ...
            var cpHost = natDestination as CheckPoint_Host;
            if (cpHost != null)
            {
                IPNetwork network;
        ...
        private List<IPNetwork> GetJuniperInterfaceRoutes(Juniper_Interface ifc)
        {
            List<IPNetwork> interfaceRoutes = null;

            if (ifc.Topology.Count > 0 && !_juniperInterfacesRoutes.TryGetValue(ifc.Name, out interfaceRoutes))
            {
                interfaceRoutes = new List<IPNetwork>();

                foreach (var route in ifc.Routes)
                {
                    IPNetwork ipNetwork;
                    if (IPNetwork.TryParse(route.IpAddress, route.Netmask, out ipNetwork) && ipNetwork.Cidr != 0)   // skip default route...
                    {
                        interfaceRoutes.Add(ipNetwork);
                    }
                }

                if (interfaceRoutes.Count > 0)
                {
                    _juniperInterfacesRoutes.Add(ifc.Name, interfaceRoutes);
                }
            }
        ...
        private void CheckObjectNameValidity(CheckPointObject cpObject, JuniperObject juniperObject, bool inMultipleZones = false)
        {
            ...
            if (_juniperDuplicateNamesLookup.TryGetValue(originalName, out duplicatesCounter))
            {
                ...
                string errorTitle = "Detected an object with a same name in Check Point's predefined service objects repository. Please review for further possible modifications to objects before migration.";
        ...
        private CheckPointObject GetCheckPointObjectOrCreateDummy(string cpObjectName, string dummyObjectType, JuniperObject juniperObject, string errorTitle, string errorDescription, bool createDummy = true)
        {
            ...
            {
                case "Host":
                    cpDummyObject = new CheckPoint_Host { Name = "_Err_in_network-line_" + juniperObject.LineNumber };
                    break;
        ...
        public override void Convert(bool convertNat)
        {
            ...
            Add_Zones();   // must be called AFTER handling interfaces and routes!!!
